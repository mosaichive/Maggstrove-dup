...
import { menTrousers, TrouserProduct } from "@/data/menTrousers";
...
const AdminPriceEditor = ({
  ...
  product: TrouserProduct & { price: number };
...
// ─── Main Page ────────────────────────────────────────────────────────────────
const MenJeansTrousersPage = () => {
  const { isAdmin } = useAuth();
  const { isAvailable } = useProductOverrides();
  const [products, setProducts] = useState<TrouserProduct[]>(menTrousers);
        ...
        <div className="bg-secondary py-10 md:py-14">
          <div className="container mx-auto px-4 text-center">
            <nav className="text-sm text-muted-foreground mb-3">
              ...
              <span className="mx-2">/</span>
              <span className="text-foreground">Jeans & Trousers</span>
            </nav>
            <h1 className="text-2xl md:text-4xl font-bold tracking-tight mb-2">Jeans & Trousers</h1>
            <p className="text-muted-foreground max-w-xl mx-auto text-sm">
              Premium joggers and track pants with designer-inspired tape details.
            </p>
          </div>
        </div>
...
export default MenJeansTrousersPage;
