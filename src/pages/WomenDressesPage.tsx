...
interface AdminPriceEditorProps {
  product: DressProduct & { price: number };
  onSave: (id: string, newPrice: number) => void;
}

const AdminPriceEditor = ({ product, onSave }: AdminPriceEditorProps) => {
      ...
      {editing ? (
        ...
        >
          ...
          <span className="group-hover:underline">
            {product.adminOverride ? "Admin override" : "Edit price"}
          </span>
        </button>
      )}
    </div>
...
const PriceBadge = ({ score }: { score: number }) => {
  const tier =
    score >= 8 ? { label: "Premium", cls: "bg-accent/10 text-accent" }
    : score >= 6 ? { label: "Mid-range", cls: "bg-secondary text-muted-foreground" }
...
const WomenDressesPage = () => {
  ...
  const handlePriceOverride = (id: string, newPrice: number) => {
    setProducts((prev) =>
      prev.map((p) =>
        p.id === id ? { ...p, price: newPrice, adminOverride: true } : p
      )
    );
  };
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1">
        ...
        <PromoBanner
          ...
          leftCta="Shop Dresses"
          leftLink="/women/dresses"
          rightImage={promoRight}
          rightTitle="SHOP NOW"
          rightCta="Explore Collection"
          rightLink="/women/dresses"
        />
        ...
        {/* Pricing methodology notice */}
        <div className="bg-muted/50 border-b border-border">
          <div className="container mx-auto px-4 py-2.5 flex items-start gap-2">
            <Info className="w-3.5 h-3.5 text-muted-foreground mt-0.5 shrink-0" />
            <p className="text-[11px] text-muted-foreground leading-relaxed">
              <strong className="text-foreground">Auto-pricing active.</strong> Each dress is priced between{" "}
              ...
              <div key={product.id} className="flex flex-col">
                ...
                <div className="mt-2 flex items-center gap-2 flex-wrap">
                  ...
                  <span className="text-[10px] text-muted-foreground">
                    Score: {product.complexityScore}/10
                  </span>
                  {product.adminOverride && (
                    <span className="text-[9px] text-accent uppercase tracking-wider font-semibold">
                      ✎ Admin
                    </span>
                  )}
                </div>
                ...
                {/* Admin Price Editor */}
                {adminMode && (
                  <AdminPriceEditor product={product} onSave={handlePriceOverride} />
                )}
              </div>
            ))}
          </div>
